...
export const queueService = {
  ...
  async getQueue(): Promise<QueueItem[]> {
    ...
    return apiClient.get<QueueItem[]>('/operator/queue');
  },
  ...
  async callNext(): Promise<QueueItem | null> {
    ...
    return apiClient.post<QueueItem>('/operator/queue/call-next');
  },
  ...
  async updateStatus(token: string, status: QueueStatus, remarks?: string): Promise<QueueItem> {
    ...
    return apiClient.put<QueueItem>(`/operator/queue/${token}/status`, { status, remarks });
  },
